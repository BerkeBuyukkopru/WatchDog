    ...
    <div className="min-h-screen bg-background-darker flex flex-col">
      ...
      <header className="h-16 bg-background border-b border-white/5 flex items-center justify-between px-4 sm:px-6 shrink-0">
          ...
          {selectedAppName && (
            <>
              ...
              <div className="hidden lg:flex items-center gap-2">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">İzlenen:</span>
                <span className="text-xs font-black text-indigo-400 uppercase tracking-tight">{selectedAppName}</span>
              </div>
            </>
          )}
        </div>
        ...
        <div className="flex items-center gap-4 sm:gap-8">
          ...
          <div className={`hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-full border shadow-lg shadow-emerald-500/5 ${apiError ? 'bg-rose-500/10 border-rose-500/20' : 'bg-emerald-500/10 border-emerald-500/20'}`}>
            <div className={`w-1.5 h-1.5 rounded-full ${apiError ? 'bg-rose-500' : 'bg-emerald-500 animate-pulse'}`}></div>
            <span className={`text-[9px] font-black uppercase tracking-[0.1em] ${apiError ? 'text-rose-500' : 'text-emerald-500'}`}>
              {apiError ? 'OFFLINE' : 'ONLINE'}
            </span>
          </div>

          {/* Kullanıcı Bilgisi - Yeniden Düzenlendi (İkon Solda) */}
          ...
          <button onClick={logout} className="flex items-center gap-2 text-[10px] sm:text-[11px] font-black uppercase tracking-[0.15em] text-rose-500 hover:text-rose-400 transition-all active:scale-95 pl-2">
      ...
      <main className="flex-1 overflow-auto custom-scrollbar p-4 sm:p-6 lg:p-8">
        <Outlet context={{ setApiError, setSelectedAppName }} />
      </main>
    </div>
  );
};
